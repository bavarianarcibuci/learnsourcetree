...
for akSet = 1:SetSize                         % and loop through all of them
    ...
    fprintf( '\n-----------------------------------\n- Rate-Distortion ANALYSING\n' );
    [ MSE ] = calcMSE( ORIGINAL_image * 256, RECONSTRUCTED_image * 256, ORIGINAL_dimensions, ORIGINAL_height, ORIGINAL_width );
    [ PSNR ] = calcPSNR( MSE );
    fprintf( '- Rate-Distortion ANALYSING\n-----------------------------------\n\n' );
    
    PeakSNR( akSet ) = PSNR;
    BitRate( akSet ) = COMPRESSED_size( 1, 1 ) * 8 / (512 ^ 2);
